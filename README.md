<h1 align="center">
  <span style="color:#ff69b4;">🌸 Seu Nome</span>
</h1>

<p align="center">
  <img src="https://img.shields.io/badge/STATUS-ONLINE-ff69b4?style=for-the-badge&logo=github">
  <img src="https://img.shields.io/badge/ÁREA-DESENVOLVIMENTO%20DE%20SISTEMAS-ff77b7?style=for-the-badge">
</p>

<!-- Banner minimalista rosa -->
<div style="height:8px; background: linear-gradient(to right, #ff9ce5, #ff69b4, #ff1493); border-radius:4px; margin:25px 0;"></div>

## Sobre mim

Sou estudante de **Desenvolvimento de Sistemas pelo SENAI**, com experiência em administração, adquirindo **organização, foco e disciplina**.  

Sou dedicada, focada e busco constantemente aprender novas tecnologias e desenvolver soluções eficientes.  

<div style="height:4px; background: linear-gradient(to right, #ff9ce5, #ff69b4, #ff1493); border-radius:3px; margin:25px 0;"></div>

## Objetivo

Atuar como **desenvolvedora de sistemas**, contribuindo com soluções práticas e evoluindo tecnicamente.  

<div style="height:4px; background: linear-gradient(to right, #ff9ce5, #ff69b4, #ff1493); border-radius:3px; margin:25px 0;"></div>

## Habilidades Técnicas

### Linguagens / Tecnologias
<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,python" />
</p>

### Desenvolvimento Web
<p>
  <img src="https://skillicons.dev/icons?i=bootstrap" />
</p>

### Banco de Dados
<p>
  <img src="https://skillicons.dev/icons?i=mysql" />
</p>

### Ferramentas
<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode" />
</p>

<div style="height:4px; background: linear-gradient(to right, #ff9ce5, #ff69b4, #ff1493); border-radius:3px; margin:25px 0;"></div>

## Competências

- 🌸 Organização e gestão de tarefas  
- 💖 Comunicação clara e objetiva  
- 🎀 Trabalho em equipe  
- ✨ Proatividade e dedicação  
- 🎓 Facilidade de aprendizado  
- 🔍 Pensamento lógico  

<div style="height:4px; background: linear-gradient(to right, #ff9ce5, #ff69b4, #ff1493); border-radius:3px; margin:25px 0;"></div>

## Aprendizado Atual

- Desenvolvimento Web  
- Boas práticas de programação  
- Lógica e resolução de problemas  
- Novas linguagens e frameworks  

<div style="height:4px; background: linear-gradient(to right, #ff9ce5, #ff69b4, #ff1493); border-radius:3px; margin:25px 0;"></div>

## Estatísticas

<p align="center">
  <img 
    src="https://github-readme-stats.vercel.app/api?username=SEU_USER&show_icons=true&title_color=ff69b4&icon_color=ff77b7&text_color=ffffff&bg_color=0d1117" 
  />
</p>

<div style="height:4px; background: linear-gradient(to right, #ff9ce5, #ff69b4, #ff1493); border-radius:3px; margin:25px 0;"></div>

## Contato

<p align="center">
  <a href="mailto:seuemail@email.com">
    <img src="https://img.shields.io/badge/Email-ff69b4?style=for-the-badge&logo=gmail&logoColor=white">
  </a>
</p>
